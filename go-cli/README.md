...
AQL-driven command-line client **and** interactive REPL for the HealthcareGovContent
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/healthcare-gov-content-cli)
make build

# 2. See usage (words, entities, env vars)
./healthcare-gov-content-cli --help
...
./healthcare-gov-content-cli load 1 content_collection            # {id:1} shorthand
./healthcare-gov-content-cli load '{id:1}' content_collection       # explicit match map

# 5. Override the API base URL for a single call
HEALTHCARE_GOV_CONTENT_BASE=https://api.example.com ./healthcare-gov-content-cli load 1 content_collection

# 6. No arguments -> interactive REPL
./healthcare-gov-content-cli
healthcare-gov-content> load 1 content_collection
healthcare-gov-content> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/healthcare-gov-content-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/healthcare-gov-content-cli load 1 content_collection
   ```
...
```sh
./healthcare-gov-content-cli load 1 content_collection          # scalar shorthand for {id:1}
./healthcare-gov-content-cli load '{id:1}' content_collection     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `healthcare-gov-content>`). Each line is
evaluated as its own AQL expression:

```text
$ ./healthcare-gov-content-cli
healthcare-gov-content> load 1 content_collection
healthcare-gov-content> /help
healthcare-gov-content> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `content_collection`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/healthcare-gov-content-cli`. |
...
(`./healthcare-gov-content-cli <expr>`) and interactively (the REPL), and expressions compose the
