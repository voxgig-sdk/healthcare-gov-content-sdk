...
local ContentCollectionEntity = {}
ContentCollectionEntity.__index = ContentCollectionEntity


function ContentCollectionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContentCollectionEntity)
  self._name = "content_collection"
  self._client = client
...
function ContentCollectionEntity:get_name()
  return self._name
end


function ContentCollectionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContentCollectionEntity.new(self._client, opts)
end


function ContentCollectionEntity:data_set(args)
  if args ~= nil then
...
function ContentCollectionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContentCollectionEntity:match_set(args)
  if args ~= nil then
...
function ContentCollectionEntity:match_get()
...
function ContentCollectionEntity:stream(action, args, callopts)
...
function ContentCollectionEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ContentCollectionEntity
