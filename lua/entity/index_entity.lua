...
local IndexEntity = {}
IndexEntity.__index = IndexEntity


function IndexEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IndexEntity)
  self._name = "index"
  self._client = client
...
function IndexEntity:get_name()
  return self._name
end


function IndexEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IndexEntity.new(self._client, opts)
end


function IndexEntity:data_set(args)
  if args ~= nil then
...
function IndexEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IndexEntity:match_set(args)
  if args ~= nil then
...
function IndexEntity:match_get()
...
function IndexEntity:stream(action, args, callopts)
...
function IndexEntity:_run_op(ctx, post_done)
  local utility = self._utility
