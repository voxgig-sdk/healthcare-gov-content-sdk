...
The Lua SDK for the HealthcareGovContent API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:ContentCollection(nil):load({ id = "example_id" }, nil)
...
local result, err = client:HealthcareGovContent(nil):load(
  { id = "test01" }, nil
)
