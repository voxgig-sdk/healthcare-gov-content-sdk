...
describe("PostTitleDirect", function()
  it("should direct-list-post_title", function()
    local setup = post_title_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-post_title", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"post_title01"}) do
...
function post_title_direct_setup(mockres)
