...
module HealthcareGovContentUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HealthcareGovContentResponse.new({})
    ...
    if fetch_err
      ...
      response = HealthcareGovContentResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HealthcareGovContentResponse.new(fetched)
    else
