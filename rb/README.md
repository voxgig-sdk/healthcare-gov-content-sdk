# HealthcareGovContent Ruby SDK

The Ruby SDK for the HealthcareGovContent API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.ContentCollection(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.HealthcareGovContent(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── HealthcareGovContent_sdk.rb       -- Main SDK module
