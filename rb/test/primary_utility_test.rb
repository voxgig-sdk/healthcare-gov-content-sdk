...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = HealthcareGovContentBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = HealthcareGovContentSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = HealthcareGovContentSDK.test(nil, nil)
  ...
  def test_fetcher_live
    calls = []
    live_client = HealthcareGovContentSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = HealthcareGovContentSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
        },
  ...
  def test_make_fetch_def_basic
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = HealthcareGovContentSpec.new({
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = HealthcareGovContentSpec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = HealthcareGovContentSDK.test(nil, setup_opts)
  ...
  def test_make_url_basic
    runset(get_spec(@primary, "makeUrl", "basic")) do |entry|
      ctxmap = entry["ctx"] || {}
      ctx = make_ctx_from_map(ctxmap, @client, @utility)
      ctx.result = HealthcareGovContentResult.new({}) unless ctx.result
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = HealthcareGovContentOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = HealthcareGovContentSDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = HealthcareGovContentContext.new(ctxmap, nil)
    ...
    if ctxmap["spec"].is_a?(Hash)
      ctx.spec = HealthcareGovContentSpec.new(ctxmap["spec"])
    end

    # Handle result from JSON map
    if ctxmap["result"].is_a?(Hash)
      res_map = ctxmap["result"]
      ctx.result = HealthcareGovContentResult.new(res_map)
      if res_map["err"].is_a?(Hash) && res_map["err"]["message"].is_a?(String)
        ctx.result.err = HealthcareGovContentError.new("", res_map["err"]["message"])
      end
    end
    ...
    if ctxmap["response"].is_a?(Hash)
      ...
      ctx.response = HealthcareGovContentResponse.new(resp_map)
      if resp_map["body"]
  ...
  def err_from_map(m)
    ...
    code = m["code"] || ""
    HealthcareGovContentError.new(code, msg)
  end
end


# === Test hook feature for featureHook test ===
class TestHookFeature < HealthcareGovContentBaseFeature
...
class TestInitFeature < HealthcareGovContentBaseFeature
