...
class IndexDirectTest < Minitest::Test
  def test_direct_list_index
    setup = index_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-index", setup[:live] ? "live" : "unit")
    if _should_skip
...
def index_direct_setup(mockres)
