...
class PostTitleDirectTest < Minitest::Test
  def test_direct_list_post_title
    setup = post_title_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-post_title", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["post_title01"].each do |_live_key|
...
def post_title_direct_setup(mockres)
