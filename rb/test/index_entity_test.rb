...
class IndexEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = HealthcareGovContentSDK.test(seed, nil)
    seen = base.Index(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = HealthcareGovContentConfig.make_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = HealthcareGovContentSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.Index(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = index_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "index." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set HEALTHCAREGOVCONTENT_TEST_INDEX_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    index_ref01_data = nil
    if index_ref01_data_raw.length > 0
...
def index_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "index", "IndexTestData.json")
  ...
  entid_env_raw = ENV["HEALTHCAREGOVCONTENT_TEST_INDEX_ENTID"]
