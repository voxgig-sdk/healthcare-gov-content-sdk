...
class ContentCollectionEntity
  ...
  def make
    opts = @_entopts.dup
    ContentCollectionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HealthcareGovContentHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ContentCollection, Hash] the current ContentCollection data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HealthcareGovContentHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ContentCollection fields)
  def match_get
  ...
  #
  # @param reqmatch [ContentCollectionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ContentCollection.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ContentCollection, Hash] the loaded ContentCollection; raises HealthcareGovContentError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = HealthcareGovContentHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
