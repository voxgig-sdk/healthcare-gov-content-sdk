...
// HealthcareGovContent SDK ContentCollection entity
...
class ContentCollectionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ContentCollectionEntity($this->_client, $opts);
    }

    /**
     * @param ContentCollection|array $args ContentCollection data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = HealthcareGovContentHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ContentCollection|array The current ContentCollection data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ContentCollection fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = HealthcareGovContentHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ContentCollection fields).
     */
    ...
    /**
     * Load a single ContentCollection.
     *
     * @param ContentCollectionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ContentCollectionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ContentCollection|array The loaded ContentCollection as an assoc-array at the
     *   SDK boundary; throws HealthcareGovContentError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = HealthcareGovContentHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
