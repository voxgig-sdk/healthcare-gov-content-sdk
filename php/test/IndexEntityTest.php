...
class IndexEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = HealthcareGovContentSDK::test($seed, null);
        $seen = iterator_to_array($base->Index(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Index(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HEALTHCAREGOVCONTENT_TEST_INDEX_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function index_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["index01", "index02", "index03"] as $k) {
    ...
    $entid_env_raw = getenv("HEALTHCAREGOVCONTENT_TEST_INDEX_ENTID");
