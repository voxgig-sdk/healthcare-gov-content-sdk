...
class ContentCollectionEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = HealthcareGovContentSDK::test(null, null);
        $ent = $testsdk->ContentCollection(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = content_collection_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "content_collection." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HEALTHCAREGOVCONTENT_TEST_CONTENT_COLLECTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $content_collection_ref01_data = null;
        if (count($content_collection_ref01_data_raw) > 0) {
...
function content_collection_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/content_collection/ContentCollectionTestData.json';
    ...
    $entid_env_raw = getenv("HEALTHCAREGOVCONTENT_TEST_CONTENT_COLLECTION_ENTID");
