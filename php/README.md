# HealthcareGovContent PHP SDK



The PHP SDK for the HealthcareGovContent API — an entity-oriented client using PHP conventions.
...
## Install
```bash
composer require voxgig-sdk/healthcare-gov-content
```
...
```php
[$result, $err] = $client->ContentCollection()->load(["id" => "example_id"]);
...
[$result, $err] = $client->HealthcareGovContent()->load(["id" => "test01"]);
...
```
php/
├── healthcaregovcontent_sdk.php          -- Main SDK class
...
The main class (`healthcaregovcontent_sdk.php`) exports the SDK class
