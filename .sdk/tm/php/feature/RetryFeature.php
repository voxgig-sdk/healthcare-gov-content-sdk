...
// HealthcareGovContent SDK retry feature
...
class HealthcareGovContentRetryFeature extends HealthcareGovContentBaseFeature
{
    ...
    public function init(HealthcareGovContentContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HealthcareGovContentContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HealthcareGovContentContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HealthcareGovContentContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
