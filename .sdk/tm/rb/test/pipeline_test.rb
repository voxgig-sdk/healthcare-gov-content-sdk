...
class PipelineTest < Minitest::Test
  ...
  def resp(status, data = nil, headers = nil)
    ...
    HealthcareGovContentResponse.new({
      "status" => status,
  ...
  def spec_of(map = {})
    HealthcareGovContentSpec.new({ "step" => "s", "method" => "GET", "headers" => {} }.merge(map))
  end
  ...
  def test_make_response_guards_missing_spec_response_result
    ...
    ctx.result = HealthcareGovContentResult.new({})
  ...
  def test_make_result_empty_list_yields_an_empty_resdata_array
    ...
    ctx.spec = spec_of
    ctx.result = HealthcareGovContentResult.new({ "ok" => true, "resdata" => [] })
    result, err = @utility.make_result.call(ctx)
    assert_nil err
  ...
  def test_make_result_short_circuits_a_preset_result
    ctx = make_ctx
    preset = HealthcareGovContentResult.new({ "ok" => true })
  ...
  def test_make_request_a_transport_error_tuple_lands_on_the_response
    boom = HealthcareGovContentError.new("boom", "boom")
  ...
  def test_make_request_a_fetchdef_error_surfaces_as_a_response_error
    u = @client.get_utility
    u.make_fetch_def = ->(ctx) { [nil, HealthcareGovContentError.new("fetchdef_boom", "boom")] }
    ctx = make_ctx(utility: u)
    ctx.spec = request_spec
    response, err = u.make_request.call(ctx)
    assert_nil err
  ...
  def test_done_returns_resdata_on_success
    ctx = make_ctx
    ctx.result = HealthcareGovContentResult.new({ "ok" => true, "resdata" => 42 })
    assert_equal 42, @utility.done.call(ctx)
  end

  def test_done_raises_the_error_when_not_ok
    ctx = make_ctx
    ctx.result = HealthcareGovContentResult.new({ "ok" => false })
    assert_raises(HealthcareGovContentError) { @utility.done.call(ctx) }
  end

  def test_done_cleans_ctrl_explain_on_success
    ctx = make_ctx
    ctx.ctrl.explain = { "result" => { "err" => "x" } }
    ctx.result = HealthcareGovContentResult.new({ "ok" => true, "resdata" => 7 })
  ...
  def test_make_error_returns_resdata_when_throw_is_disabled
    ...
    ctx.result = HealthcareGovContentResult.new({ "ok" => false, "resdata" => "fallback" })
  ...
  def test_make_error_preserves_the_error_code
    ctx = make_ctx
    err = assert_raises(HealthcareGovContentError) {
  ...
  def test_feature_add_appends_in_call_order
    ...
    client.features = []
    a = HealthcareGovContentBaseFeature.new
    b = HealthcareGovContentBaseFeature.new
  ...
  def named_feature(name)
    f = HealthcareGovContentBaseFeature.new
    f.name = name
    f
  end
  ...
  def auth_ctx(options, headers)
    ctx = HealthcareGovContentContext.new({
      "client" => OptClient.new(options),
      "utility" => @utility,
      "opname" => "load",
    }, nil)
    ctx.spec = headers.nil? ? nil : HealthcareGovContentSpec.new({ "headers" => headers, "step" => "s" })
    ctx
  end
  ...
  def test_result_headers_with_non_hash_headers_yields_an_empty_map
    ctx = make_ctx
    ctx.response = HealthcareGovContentResponse.new({ "status" => 200 })
    ctx.result = HealthcareGovContentResult.new({})
  ...
  def test_result_body_skips_parsing_when_the_body_is_absent
    ctx = make_ctx
    ctx.response = HealthcareGovContentResponse.new({
