...
The Python SDK for the HealthcareGovContent API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.ContentCollection(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.HealthcareGovContent(None).load(
    {"id": "test01"}, None
)
