...
class TestPostTitleDirect:

    def test_should_direct_list_post_title(self):
        setup = _post_title_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-post_title", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["post_title01"]:
...
def _post_title_direct_setup(mockres):
