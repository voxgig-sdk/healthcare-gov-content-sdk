...
class TestContentCollectionDirect:

    def test_should_direct_load_content_collection(self):
        setup = _content_collection_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-content_collection", "live" if setup["live"] else "unit")
        if _skip:
...
def _content_collection_direct_setup(mockres):
