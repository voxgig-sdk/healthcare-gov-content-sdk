...
class TestIndexDirect:

    def test_should_direct_list_index(self):
        setup = _index_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-index", "live" if setup["live"] else "unit")
        if _skip:
...
def _index_direct_setup(mockres):
