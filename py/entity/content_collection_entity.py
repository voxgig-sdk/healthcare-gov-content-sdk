...
from healthcaregovcontent_types import (
    ContentCollection,
...
class ContentCollectionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ContentCollectionEntity(self._client, opts)
    ...
    def data_get(self) -> ContentCollection:
    ...
    def match_get(self) -> ContentCollection:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> ContentCollection:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ContentCollection().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
