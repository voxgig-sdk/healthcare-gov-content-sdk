...
[MCP](https://modelcontextprotocol.io) server exposing the HealthcareGovContent SDK as
two agent tools — `healthcare-gov-content_list` and `healthcare-gov-content_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/healthcare-gov-content-mcp)
make build

# 2. Provide credentials via the environment
export HEALTHCARE_GOV_CONTENT_APIKEY=sk_live_xxx
...
```jsonc
// healthcare-gov-content_list: first page of records
{ "entity": "index" }
{ "entity": "index", "query": { } }

// healthcare-gov-content_load: one record by id
{ "entity": "content_collection", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/healthcare-gov-content-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user healthcare-gov-content \
     -- "$PWD"/dist/*/healthcare-gov-content-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `healthcare-gov-content_list` and `healthcare-gov-content_load` tools now appear
   in new sessions. Ask the agent to *"list index using healthcare-gov-content"*
   and it calls `healthcare-gov-content_list` with `{"entity":"index"}`.
...
```sh
export HEALTHCARE_GOV_CONTENT_APIKEY=sk_live_xxx            # API key
export HEALTHCARE_GOV_CONTENT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `healthcare-gov-content_list` tool
...
### Call the `healthcare-gov-content_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `healthcare-gov-content_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `healthcare-gov-content_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HEALTHCARE_GOV_CONTENT_APIKEY` | API key sent with every request. |
| `HEALTHCARE_GOV_CONTENT_BASE` | Optional override of the API base URL. |
