...
type Args struct {
	Entity string         `json:"entity" jsonschema:"content_collection | index | post_title"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.HealthcareGovContentSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "healthcare-gov-content_list",
		Description: "List records from HealthcareGovContent. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "healthcare-gov-content_load",
		Description: "Load a single record from HealthcareGovContent. " +
...
func runOp(client *sdk.HealthcareGovContentSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.HealthcareGovContentSDK, name string) (sdk.HealthcareGovContentEntity, error) {
	switch strings.ToLower(name) {
	case "content_collection":
		return client.ContentCollection(nil), nil
	case "index":
		return client.Index(nil), nil
	case "post_title":
		return client.PostTitle(nil), nil
