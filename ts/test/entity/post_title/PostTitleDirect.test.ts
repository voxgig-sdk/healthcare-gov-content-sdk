...
describe('PostTitleDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new HealthcareGovContentSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-list-post_title', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-post_title', setup.live)) return
    if (skipIfMissingIds(t, setup, ["post_title01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HEALTHCAREGOVCONTENT_TEST_POST_TITLE_ENTID']
