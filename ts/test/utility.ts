...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.HEALTHCAREGOVCONTENT_TEST_LIVE ||
    'TRUE' === process.env.HEALTHCAREGOVCONTENT_TEST_OVERRIDE
  ) {
  ...
  m.HEALTHCAREGOVCONTENT_TEST_EXPLAIN = process.env.HEALTHCAREGOVCONTENT_TEST_EXPLAIN || m.HEALTHCAREGOVCONTENT_TEST_EXPLAIN
