...
//     (\`new HealthcareGovContentSDK(...)\` / \`HealthcareGovContentSDK.test(...)\` become
//     \`HealthcareGovContentSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'HealthcareGovContentSDK'
...
const TEST_SEED = {"entity":{"content_collection":{"test01":{"id":"test01"}},"index":{"test01":{"id":"test01"}},"post_title":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HealthcareGovContentSDK(...)\` and any
// \`HealthcareGovContentSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HealthcareGovContentSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HealthcareGovContentSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
